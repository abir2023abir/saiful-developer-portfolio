  ...
  return (
    <ActionForm action={saveProject} className="space-y-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        />
        <Field label="Category" name="category" defaultValue={p.category} placeholder="SaaS Dashboard" />
      ...
      <div className="border border-black/10 bg-white p-5">
        ...
        <p className="mt-0.5 text-xs text-black/55">
          Shown on the project card and at the top of the case study. Without one the card falls back
          to the gradient below.
        </p>

        <div className="mt-4 flex flex-wrap items-start gap-5">
          <div className="relative h-28 w-44 shrink-0 overflow-hidden border border-black/10 bg-smoke">
            {image ? (
              <Image src={image} alt="" fill sizes="176px" className="object-cover object-top" />
            ) : (
              <span
          ...
          <div className="flex-1 space-y-3">
            ...
            <p aria-live="polite" className="text-xs">
              ...
              {uploadError && <span className="text-brand-ink">{uploadError}</span>}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <label className="block">
            <span className="text-sm font-semibold">Fallback gradient — from</span>
            <input name="tintFrom" type="color" defaultValue={p.tint[0]} className={`${inputClass} h-11 p-1`} />
          </label>
          <label className="block">
            <span className="text-sm font-semibold">Fallback gradient — to</span>
            <input name="tintTo" type="color" defaultValue={p.tint[1]} className={`${inputClass} h-11 p-1`} />
          </label>
        </div>
      </div>

      <div className="grid gap-6 sm:grid-cols-2">
        <Field label="Live URL" name="liveUrl" defaultValue={p.liveUrl} placeholder="https://…" />
        <Field label="Repository URL" name="repoUrl" defaultValue={p.repoUrl} placeholder="https://github.com/…" />
      </div>
      ...
      <fieldset className="space-y-6 border border-black/10 bg-white p-5">
        ...
        <p className="text-xs text-black/55">
          Each filled section becomes a block on /work/{p.slug || "your-slug"}. Leave them empty and
          the page shows the summary alone.
        </p>
        <Field label="The problem" name="problem" rows={3} defaultValue={p.problem} />
        <Field label="The approach" name="approach" rows={3} defaultValue={p.approach} />
        <Field label="The result" name="result" rows={3} defaultValue={p.result} />
      </fieldset>
      ...
      <label className="flex items-center gap-3">
        ...
        <span className="text-sm">
          ...
          <span className="ml-2 text-black/55">shown in the floating card on the home page</span>
        </span>
      </label>
    </ActionForm>
  );
